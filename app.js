...
async function handleQuoteSubmission(e) {
    ...
    // First save to Firebase, then add to local state if successful
    // This prevents duplicate quotes and ensures server consistency
    try {
        // Show loading indicator
        elements.loading.style.display = 'block';
        
        // First save to Firebase
        const serverSaveSuccessful = await saveQuoteToFile(newQuote);
        ...
        showToast(serverSaveSuccessful ? "Quote added successfully to Firebase!" : "Quote added to local storage only. Firebase unavailable.");
    } catch (error) {
...
async function loadQuotes() {
    try {
        ...
        // Try to fetch quotes from Firebase through our server API
        try {
            const response = await fetch('/api/quotes');
            if (response.ok) {
                const data = await response.json();
                state.quotes = data;
                console.log('Quotes loaded from Firebase successfully!');
                ...
                throw new Error('Failed to fetch quotes from Firebase');
            }
        } catch (serverError) {
...
async function saveQuoteToFile(newQuote) {
    try {
        ...
        try {
            ...
            // Add the Firebase ID to the quote
            if (responseData.quote && responseData.quote.id) {
                newQuote.id = responseData.quote.id;
            }
            
            console.log('Quote saved successfully to Firebase!');
            serverSaveSuccessful = true;
        } catch (serverError) {
            console.warn("Could not save to Firebase, using local storage fallback:", serverError);
            ...
            try {
                ...
                serverSaveSuccessful = true;
                console.log('Quote saved successfully to Firebase via fallback!');
            } catch (fallbackError) {
                console.error("Fallback save to Firebase failed:", fallbackError);
            }
        }
...
async function deleteQuote(quoteId) {
    ...
    try {
        ...
        try {
            ...
            // If server delete failed, we'll update the local state and try to sync later
            serverDeleteSuccessful = false;
        }
...
async function handleUndo() {
    try {
        ...
        // Get the last deleted quote
        const lastDeleted = state.deletedQuotes.pop();
        ...
        for (let i = 0; i < state.quotes.length; i++) {
            if (state.quotes[i].ref === lastDeleted.quote.ref) {
                quoteExists = true;
                existingQuoteIndex = i;
                break;
            }
        ...
        // Create statement object if needed
        const statement = {
            statement: lastDeleted.quote.statement,
            tags: lastDeleted.quote.tags || [],
            keywords: lastDeleted.quote.keywords || [],
            id: lastDeleted.quote.id
        };
        ...
        if (quoteExists) {
            ...
            // Create new quote object
            newQuote = {
                ref: lastDeleted.quote.ref,
                speaker: lastDeleted.quote.speaker || "",
                date: lastDeleted.quote.date || "",
                location: lastDeleted.quote.location || "",
                lecture: lastDeleted.quote.lecture || "",
                statements: [statement],
                id: lastDeleted.quote.id // Preserve Firebase ID if it exists
            };
            state.quotes.push(newQuote);
        }
        
        // Add back to flatQuotes
        state.flatQuotes.push(lastDeleted.quote);
        
        // Try to save to Firebase
        let serverUpdateSuccessful = false;
        try {
            // If the quote has an ID, we need to add it back to Firebase
            if (lastDeleted.quote.id) {
                const response = await fetch('/api/quotes', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                ...
                console.log('Quote restored successfully to Firebase!');
                serverUpdateSuccessful = true;
            } else {
                // If no ID, we need to save all quotes to ensure consistency
                await saveQuotesToServer();
                serverUpdateSuccessful = true;
            }
        } catch (serverError) {
            console.warn("Could not restore to Firebase, using local storage only:", serverError);
        ...
        // Hide the undo button if no more deleted quotes
        if (state.deletedQuotes.length === 0) {
            elements.undoBtn.style.display = 'none';
        }
        
        elements.loading.style.display = 'none';
        showToast(serverUpdateSuccessful ? "Quote restored successfully to Firebase!" : "Quote restored to local storage only. Firebase unavailable.");
    } catch (error) {
        console.error("Error restoring quote:", error);
        elements.loading.style.display = 'none';
        showToast("Error restoring quote. Please try again.", "error");
    }
}
...
// Function to refresh quotes from Firebase
async function refreshQuotes() {
    try {
        ...
        try {
            const response = await fetch('/api/quotes');
            if (!response.ok) {
                throw new Error(`Failed to load quotes from Firebase: ${response.status}`);
            }
            
            // Replace state.quotes with the fresh data from Firebase
            const freshQuotes = await response.json();
            state.quotes = freshQuotes;
            console.log('Quotes refreshed from Firebase successfully!');
        } catch (serverError) {
            console.warn("Could not refresh from Firebase, using local JSON fallback:", serverError);
            
            // Fallback to local JSON file if Firebase fetch fails
            const response = await fetch("quotes.json?v=" + new Date().getTime());
            if (!response.ok) throw new Error(`Failed to load quotes: ${response.status}`);
            
            // Replace state.quotes with the fresh data from local file
            const freshQuotes = await response.json();
            state.quotes = freshQuotes;
            console.log('Quotes refreshed from local JSON file!');
        }
        
        // Process the quotes to update the UI
        processQuotes();
